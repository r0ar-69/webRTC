...
//! \~japanese 映像スケーリングタイプ列挙子
//! \~english Scaling type enum
//! \~
typedef NS_ENUM(NSUInteger, SKWVideoScalingEnum)
{
    //! \~japanese アスペクト比を維持したまま映像全体が収まるようにスケーリングします。(ビューと映像のアスペクト比が異なる場合は空白が生まれます)
    //! \~english Video keep aspect and fit to view.
    //! \~
    SKW_VIDEO_SCALING_ASPECT_FIT,
    //! \~japanese アスペクト比を維持したまま空白が生まれないようにスケーリングします。(ビューと映像のアスペクト比が異なる場合はクリッピングされます)
    //! \~english Video keep aspect and fill to view.
    //! \~
...
//! \~japanese 映像スケーリングタイプ
//! \~english Scaling type
//! \~
@property (nonatomic) SKWVideoScalingEnum scaling;
